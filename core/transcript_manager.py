...
# ============================================================
# 以下为 douyin-fetcher-standalone 独有 ASR 音轨转写逻辑
# ============================================================
...
def run_whisper_transcription(audio_path: Path, output_dir: Path, whisper_path: str = "") -> bool:
  ...
  """
  from utils.logger import setup_logger
  log = setup_logger("TranscriptManager")
  transcript_path = output_dir / "transcript.md"
  
  if whisper_path:
    custom_script = Path(whisper_path)
    if custom_script.is_file():
      log.info(f"使用用户指定脚本启动 ASR: {custom_script}")
      cmd = ["bash", str(custom_script), str(audio_path), str(output_dir)]
      try:
        subprocess.run(cmd, check=True)
        return True
      except Exception as e:
        log.error(f"指定的 ASR 脚本执行失败: {e}")
        return False

  whisper_bin = shutil.which("whisper")
  if whisper_bin:
    try:
      log.info("检测到全局 whisper 指令，开始在后台运行 ASR 转写流程...")
      ...
      log.error(f"Whisper ASR 转写运行时发生异常: {e}")
  else:
    log.warning("系统 PATH 中没有发现可用的全局 whisper 命令行工具，跳过语音转写。")
...
def finalize_transcript(
  ...
  *,
  ...
  if skip_asr or not audio_path.exists():
    ...
    log.info("已跳过 ASR，下载状态记为 success。")
    return
  ...
  if asr_success:
    ...
    sample_status["status"] = "success"
    log.info("语音识别成功生成 transcript.md")
  else:
    ...
    log.warning("语音识别失败或跳过，已写入空占位文件。")
