...
> ⚠️ **免责声明**：本项目仅限学术研究与技术交流，使用者需自行承担因使用本项目所产生的全部法律风险与责任。详情参见底部的[免责声明](#免责声明)。
...
本项目仅用于学术研究、技术交流以及个人数据备份与合理化管理，请在遵守中国国内相关法律法规的前提下使用。

- **合规承诺**：使用者在使用本项目时，必须严格遵守《中华人民共和国网络安全法》、《数据安全法》、《计算机信息网络国际联网安全保护管理办法》以及《著作权法》等国内相关法律法规。
- **合理使用**：本项目仅限个人技术研究与合法备份之用。严禁将本项目以任何形式用于侵犯他人隐私、侵犯版权、进行商业盈利、爬取敏感敏感信息或进行任何其他违法违规用途。
- **风险自担**：使用者应对其因使用本项目所产生的全部行为与后果承担全部法律及技术责任。作者不对使用者的行为做出任何形式的担保，亦不对任何直接或间接引起的损失或法律纠纷承担任何责任。
- **接口变更风险**：本项目所依赖的所有接口皆源自浏览器正常渲染和公开网络通信规约。如果由于平台规则、接口升级或网络反制等导致项目功能受限或失效，属于正常技术风险。

如果你继续使用本项目，即视为你已阅读、完全理解并同意上述全部声明内容。
